class EmployeeDetail < ApplicationRecord
  ...
  scope :l1_pending_records, -> { where(status: %w[pending returned]) }
  scope :l3_pending_records, -> { where(status: ['l2_approved']) }

  # Scopes for L1, L2, L3 filtering with includes
  scope :for_l1_user, lambda { |employee_code_or_email|
  ...
  scope :for_l2_user, lambda { |employee_code|
    where('TRIM(l2_code) = ? OR l2_employer_name = ?', employee_code.strip, employee_code)
      .includes(user_details: [:department, :activity, { achievements: :achievement_remark }])
  }

  scope :for_l3_user, lambda { |employee_code|
    where('TRIM(l3_code) = ? OR l3_employer_name = ?', employee_code.strip, employee_code)
      .includes(user_details: [:department, :activity, { achievements: :achievement_remark }])
  }

  scope :with_l1_approved_achievements, lambda {
    joins(user_details: :achievements)
      .where(achievements: { status: %w[l1_approved l2_approved l2_returned] })
      .distinct
  }

  scope :with_l2_approved_achievements, lambda {
    joins(user_details: :achievements)
  ...
  def sync_to_department
    return unless department.present?

    # Normalize and Clean (similar to migration 20260131063910)
    normalized_v = department.strip
